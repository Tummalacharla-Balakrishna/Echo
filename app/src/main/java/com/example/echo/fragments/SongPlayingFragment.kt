...
class SongPlayingFragment : Fragment() {
    object Statified {
        ...
        fun changeSeekBar(){
            seekBar?.setProgress(mediaPlayer?.currentPosition as Int)
            if(currentSongHelper?.isPlaying as Boolean){
                var currentSongPlaying  = object :Runnable{
                    override fun run() {
                        changeSeekBar()
                    }
                }
    ...
    override  fun onPrepareOptionsMenu(menu: Menu?) {
        ...
        val item: MenuItem?=menu?.findItem(R.id.action_redirect)
                item?.isVisible=true
        val item2: MenuItem?=menu?.findItem(R.id.action_sort)  //Need to be changed
